...
// v7 rename v- directives to m- for minnow
...
class Component {
    ...
    getForVariables(template) {
        const forVars = new Set();
        const matches = template.matchAll(/m-for="(\w+)\s+in\s+/g);
        for (const match of matches) {
            forVars.add(match[1]);
        }
        return forVars;
    ...
    resolveMustaches(container) {
        ...
        const walk = (node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
                if (node.style && node.style.display === 'none') return;
                if (node.hasAttribute && node.hasAttribute('m-for')) return;
    ...
    processDirectives(container) {
        this.processIf(container);
        this.processFor(container);
        this.processHtml(container);
        this.processBind(container);
        this.processModel(container);
    }

    processIf(container) {
        container.querySelectorAll('[m-if]').forEach(el => {
            const show = this.evaluateExpression(el.getAttribute('m-if'));
            el.style.display = show ? '' : 'none';
    ...
    processFor(container) {
        container.querySelectorAll('[m-for]').forEach(el => {
            const match = el.getAttribute('m-for').match(/(\w+)\s+in\s+(.+)/);
            if (!match) return;
            ...
            const templateHtml = el.outerHTML.replace(/m-for="[^"]*"/, '');
    ...
    processHtml(container) {
        container.querySelectorAll('[m-html]').forEach(el => {
            const expr = el.getAttribute('m-html');
            const value = this.evaluateExpression(expr);
            el.innerHTML = value != null ? String(value) : '';
            el.removeAttribute('m-html');
        });
    }

    processBind(container) {
        container.querySelectorAll('[\\:class], [m-bind\\:class]').forEach(el => {
            const expr = el.getAttribute(':class') || el.getAttribute('m-bind:class');
            if (!expr) return;
            const classObj = this.evaluateExpression(expr);
            if (typeof classObj === 'object') {
                Object.keys(classObj).forEach(cls => {
            ...
            el.removeAttribute(':class');
            el.removeAttribute('m-bind:class');
        });
    }

    processModel(container) {
        container.querySelectorAll('[m-model]').forEach(el => {
    ...
    bindEvents(container) {
        container.querySelectorAll('[\\@click], [m-on\\:click]').forEach(el => {
            const handler = el.getAttribute('@click') || el.getAttribute('m-on:click');
            el.removeAttribute('@click');
            el.removeAttribute('m-on:click');
            el.addEventListener('click', (e) => {
                const match = handler.match(/^(\w+)\((.*)\)$/);
                if (match) {
                    const args = match[2]
